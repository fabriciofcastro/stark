...
interface SmartChatbotProps {
  onContactRequest: (data: { name: string; email: string; phone: string; service: string }) => void;
  onWhatsAppRedirect: (message: string) => void;
}

const SmartChatbot = ({ onContactRequest, onWhatsAppRedirect }: SmartChatbotProps) => {
  ...
  const conversationFlow = [
    {
      id: "welcome",
      message: "Olá! 👋 Sou o assistente virtual da STARK. Como posso ajudar você hoje?",
      options: [
        { id: "suporte", text: "Preciso de suporte técnico", action: "service" },
        { id: "consultoria", text: "Quero consultoria estratégica", action: "service" },
    ...
    {
      id: "name",
      message: "Obrigado, {name}! Agora me informe seu e-mail para que possamos entrar em contato:",
      action: "get_email"
    },
    {
      id: "email",
    ...
    {
      id: "phone",
      message: "Perfeito! Com base nas suas informações, posso te ajudar de 3 formas:",
      options: [
        { id: "whatsapp", text: "💬 Conversar no WhatsApp agora", action: "whatsapp" },
  ...
  // Simular digitação
  const simulateTyping = useCallback((_message: string, callback: () => void) => {
    setIsTyping(true);
    setTimeout(() => {
      setIsTyping(false);
      callback();
                        ...
                        <div className="mt-2 space-y-1">
                          {message.options.map((option) => (
                            <button
                              key={option.id}
                              type="button"
                              onClick={() => handleUserResponse(option.text, option.action)}
